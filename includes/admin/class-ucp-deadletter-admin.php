...
/**
 * UCP_Deadletter_Admin — submenu + AJAX handlers for the dead-letter queue.
 */
final class UCP_Deadletter_Admin {
	...
	public function register_menu(): void {
		add_submenu_page(
			'ucp-for-woocommerce',
			__( 'Failed Webhooks', 'ucp-for-woocommerce' ),
			__( 'Failed Webhooks', 'ucp-for-woocommerce' ),
			'manage_woocommerce',
	...
	public function render_page(): void {
		if ( ! current_user_can( 'manage_woocommerce' ) ) {
			wp_die( esc_html__( 'You do not have permission to view this page.', 'ucp-for-woocommerce' ) );
		}
		...
		?>
		<div class="wrap">
			<h1><?php esc_html_e( 'Failed Webhooks', 'ucp-for-woocommerce' ); ?></h1>
			<p>
				<?php
				esc_html_e(
			...
			<?php if ( empty( $rows ) ) : ?>
				<div class="notice notice-success inline">
					<p><?php esc_html_e( 'No failed webhook deliveries.', 'ucp-for-woocommerce' ); ?></p>
				</div>
			<?php else : ?>
				<table class="wp-list-table widefat fixed striped" id="ucp-deadletter-table">
					<thead>
						<tr>
							<th scope="col"><?php esc_html_e( 'Failed', 'ucp-for-woocommerce' ); ?></th>
							<th scope="col"><?php esc_html_e( 'Subscription', 'ucp-for-woocommerce' ); ?></th>
							<th scope="col"><?php esc_html_e( 'Event', 'ucp-for-woocommerce' ); ?></th>
							<th scope="col"><?php esc_html_e( 'Attempts', 'ucp-for-woocommerce' ); ?></th>
							<th scope="col"><?php esc_html_e( 'Last error', 'ucp-for-woocommerce' ); ?></th>
							<th scope="col"><?php esc_html_e( 'Actions', 'ucp-for-woocommerce' ); ?></th>
						</tr>
					</thead>
					<tbody>
						<?php foreach ( $rows as $row ) : ?>
							...
							$ago        = $failed_ts
								? sprintf(
									...
									esc_html__( '%s ago', 'ucp-for-woocommerce' ),
									human_time_diff( $failed_ts, time() )
								)
								: esc_html__( 'unknown', 'ucp-for-woocommerce' );
						...
						<em>
							...
							echo esc_html(
								sprintf(
									...
									__( 'Showing %1$d of %2$d total — use WP-CLI for the full list.', 'ucp-for-woocommerce' ),
									self::PAGE_LIMIT,
									$total
								)
							);
				...
				(function () {
					...
					var msgRetry = <?php echo wp_json_encode( __( 'Retry this delivery?', 'ucp-for-woocommerce' ) ); ?>;
					var msgDrop  = <?php echo wp_json_encode( __( 'Discard this row permanently?', 'ucp-for-woocommerce' ) ); ?>;
					var msgFail  = <?php echo wp_json_encode( __( 'Action failed.', 'ucp-for-woocommerce' ) ); ?>;
	...
	public function ajax_retry(): void {
		...
		if ( $row_id <= 0 ) {
			wp_send_json_error( array( 'message' => __( 'Missing or invalid row id.', 'ucp-for-woocommerce' ) ), 400 );
		}

		$affected = self::retry_row( $row_id );
		if ( $affected < 1 ) {
			wp_send_json_error( array( 'message' => __( 'Row not found.', 'ucp-for-woocommerce' ) ), 404 );
		}
