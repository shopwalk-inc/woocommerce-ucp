...
class UCP_Webhook_Deadletter_CLI { // phpcs:ignore Generic.Files.OneObjectStructurePerFile.MultipleFound
	...
	public function deadletter_list( array $args, array $assoc_args ): void {
		...
		if ( empty( $rows ) ) {
			WP_CLI::line( __( 'No failed webhook deliveries.', 'ucp-for-woocommerce' ) );
			return;
		}
	...
	public function deadletter_retry( array $args, array $assoc_args ): void {
		$target = $args[0] ?? '';
		if ( '' === $target ) {
			WP_CLI::error( __( 'Pass a queue row id, or "all".', 'ucp-for-woocommerce' ) );
		}

		if ( 'all' === $target ) {
			$n = UCP_Deadletter_Admin::retry_all();
			...
			WP_CLI::success(
				sprintf(
					...
					_n( 'Requeued %d row.', 'Requeued %d rows.', $n, 'ucp-for-woocommerce' ),
					$n
				)
			);
			return;
		...
		$row_id = (int) $target;
		if ( $row_id <= 0 ) {
			WP_CLI::error( __( 'Invalid row id.', 'ucp-for-woocommerce' ) );
		}
		$n = UCP_Deadletter_Admin::retry_row( $row_id );
		if ( $n < 1 ) {
			WP_CLI::error(
				sprintf(
					/* translators: %d: queue row id. */
					__( 'Row %d not found (or already live).', 'ucp-for-woocommerce' ),
					$row_id
				)
			);
		}
		...
		WP_CLI::success(
			sprintf(
				/* translators: %d: queue row id. */
				__( 'Row %d requeued.', 'ucp-for-woocommerce' ),
				$row_id
			)
		);
	}
	...
	public function deadletter_discard( array $args, array $assoc_args ): void {
		...
		if ( 'all' === $target ) {
			$n = UCP_Deadletter_Admin::discard_all();
			WP_CLI::success(
				sprintf(
					/* translators: %d: number of rows deleted. */
					_n( 'Discarded %d row.', 'Discarded %d rows.', $n, 'ucp-for-woocommerce' ),
					$n
				)
			);
			return;
		...
		$n = UCP_Deadletter_Admin::discard_row( $row_id );
		if ( $n < 1 ) {
			WP_CLI::error(
				sprintf(
					/* translators: %d: queue row id. */
					__( 'Row %d not found.', 'ucp-for-woocommerce' ),
					$row_id
				)
			);
		}
		WP_CLI::success(
			sprintf(
				/* translators: %d: queue row id. */
				__( 'Row %d discarded.', 'ucp-for-woocommerce' ),
				$row_id
			)
		);
	}
